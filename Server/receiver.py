import socket, cv2, numpy as np, gzip, io
...
def handshake() -> tuple[str, str]:
    ...
    srv.listen(1)
    print(f"[PC] Waiting for Unity handshake on {TCP_IP}:{TCP_PORT}…")
    conn, _ = srv.accept()

    mobile_ip = conn.recv(1024).decode("ascii").strip()
    print(f"[PC]  ▶  got mobile IP  : {mobile_ip}")

    pc_ip = _pc_ip_towards(mobile_ip)
    conn.send(pc_ip.encode("ascii"))
    print(f"[PC]  ▶  sent back PC IP: {pc_ip}")
...
print(f"[PC] Listening for UDP packets on {pc_ip}:{UDP_PORT}…")

while True:
    packet, _ = udp_sock.recvfrom(65535)

    # -- split header (8 ';' → 9 parts) -------------------------------------
    ...
    try:
        # Read as uint16 and normalize to [0, 1] float for display
